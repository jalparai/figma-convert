...
import { deleteOptionById } from "./option-setting/api";
import { deleteCategoryById } from "../pages/categorey/api";
import { deleteSectionById } from "../pages/section/api";
import { deleteUserById } from "../pages/users/api";
const style = {
  height:'auto',
...
interface BasicProps {
  productId?: string;
  object?: "option" | "product" | "category" | "section" | "user";
}

const DeleteMod: React.FC<BasicProps> = ({ productId, object }) => {
  ...
  const handleClose = async () => {
    if(object === "option"){
      await deleteOptionById(productId);
    }
    else if(object === "product"){
      await deleteProductById(productId);
    }
    else if(object ==="category"){
      await deleteCategoryById(productId)
    }
    else if(object === "section") {
      await deleteSectionById(productId)
    }else if(object === "user"){
      await deleteUserById(productId);
    }
    else{
      throw new Error("Error wrong options!");
    }
    setOpen(false);
